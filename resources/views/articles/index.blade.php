<x-app-layout xData='{ viewType: "grid", filterOpen: {{ $filtre }}, filter : {{ $filtre }}, articles : {!! $articles !!}, deleteAction : function (id){
	...
	@endif
	<div class="mx-auto max-w-7xl pb-5 border-t border-gray-500 sm:py-12">
			...
			<div class="overflow-hidden_ flex bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
				...
				<div class="m-4">
					<button :class="filterOpen ? 'bg-green-600 hover:bg-green-800' : 'bg-indigo-600 hover:bg-indigo-800'"
						class="block rounded-md px-3 sm:px-7 py-2 text-center font-semibold text-white"
						type="button"
						x-on:click='filterOpen = ! filterOpen'>Filtre</button>
				</div>
				<div class="mr-4 mt-4">
					<button class="block rounded-md bg-indigo-600 px-3 sm:px-7 py-2 text-center font-semibold text-white hover:bg-indigo-800"
						type="button"
						x-on:click='window.location = "{{ route("articles.create") }}" '>Creer <span class="hidden sm:inline">un article</span></button>
				</div>
			</div>
			<form action=""
				class="mt-4 flex flex-col sm:flex-row justify-between gap-2 bg-white p-4 shadow-sm dark:bg-gray-800 sm:rounded-lg"
				method="get"
				x-cloak
				x-init="$el.removeAttribute('x-cloak')"
				x-show="filterOpen {{ $filtre == "false" ? "|| filter" : "" }}"
				x-transition>
				<div class="flex gap-2">
					<!-- Category -->
					<div class="grow">
						<select autocomplete="category"
							class="@error("category") ring-red-500 @else ring-gray-300 @enderror block w-full rounded-md border-0 bg-transparent px-7 py-1.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset focus:bg-gray-100 focus:text-gray-800 focus:ring-2 focus:ring-inset focus:ring-indigo-600 dark:text-gray-200 sm:max-w-xs sm:text-sm sm:leading-6"
							id="categorySelect"
							name="category"
							x-init="setCategory()"
					...
					<div class="grow">
						<select {{-- x-init="setSubCategory()" --}}
							...
							name="sub_category">
							<template 
								x-for="subCat in subCategories.filter((el) => el.category_id == curentCategory)">
				...
				<div>
					<div class="flex gap-4 justify-end mt-3 sm:mt-0">
						<button class="block rounded-md bg-red-600 px-2 py-2 text-center font-semibold text-white hover:bg-red-800"
							type="submit"
							x-on:click.prevent="location = '{{ route("articles.index") }}'"
							x-show="filter">Suprimer <span class="hidden sm:inline">le filtre</span></button>
						<button class="block rounded-md bg-indigo-600 px-4 sm:px-7 py-2 text-center font-semibold text-white hover:bg-indigo-800"
	...
	<template x-if="viewType == 'list'">
		<div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
			<div class="rounded-xl bg-white pt-8 mx-4 sm:mx-0 dark:bg-gray-800">
				<div class="max-w-7xl">
					<div class="px-4">
									...
									<table class="ts acc ach min-w-full">
										<thead>
											<tr class="border-b border-gray-500 dark:border-gray-600 dark:text-gray-200">
												<th class="    cgi text-left"
													scope="col">Nom</th>
												<th class="hidden  md:table-cell text-left"
													scope="col">Categorie</th>
												<th class="  text-left"
													scope="col">Sous-categorie</th>
												<th class="ab  atm aue cgp text-left"
										...
										<tbody class="acc acg">
											<template x-for="article in articles">
												<tr class="border-b border-gray-400 py-10 dark:border-gray-600">
													...
													<td class="py-4 dark:text-gray-300 hidden  md:table-cell"
														x-text="article.category">Categorie</td>
													<td class="py-4 dark:text-gray-300"
														x-text="article.subCategory">Smpartphone Iphone</td>
													<td class="flex flex-col md:flex-row justify-center gap-3 py-4 dark:text-indigo-300">
											...
											<template x-if="{{ $count }} == 0">
												<tr class="py-10">
													<td class="py-4 text-center font-semibold dark:text-gray-400"
														colspan="6"
	...
	<template x-if="viewType == 'grid'">
		<div class="mx-auto grid max-w-7xl grid-cols-2 px-4 sm:gap-4 sm:px-6 md:grid-cols-4 lg:px-8">
			<template x-for="article in articles">
				<div class="group m-2 gap-3 shadow-md shadow-gray-200 dark:shadow-gray-900 overflow-hidden border border-gray-300 dark:border-gray-500 rounded-lg bg-white dark:bg-gray-700 sm:m-0">
				<span class0="hidden shadow-gray-300 shadow-gray-400 shadow-gray-500 shadow-gray-600 shadow-gray-700 shadow-gray-800 shadow-gray-900" ></span>
					...
					<a :href="article.show_link">
						...
						<div class="leText">
							<h4 class="px-3 pt-2 font-semibold text-xs sm:text-base dark:text-gray-200"
								x-text="article.name"></h4>
							<p :title="'De la categorie \'' + article.category + '\'.'"
								class="px-3 pb-3 dark:text-gray-400  text-xs sm:text-sm"
								class=""
